...
#TK01 Тест авторизации с валидными значениями e-mail и паролЬ.
...
#TK02,03 "Проверка входа пользователя с невалидным email и паролем:
...
#TK04 Проверка названия табов в меню выбора типа авторизации.
    try:
        page = AuthPage(selenium)
...
#TK05 Проверка выбора таба по умолчанию в Меню выбора типа авторизации.
    page = AuthPage(selenium)
...
#TK06 Тест смены полей ввода при смене типа авторизации.
    page = AuthPage(selenium)
...
#TR07 Переход к форме "Восстановление пароля".
    page = AuthPage(selenium)
...
#TK08 Переход к форме "Регистрация".
    page = AuthPage(selenium)
...
#TK09 Проверка блока с логотипон/названием компании на странице "Регистрация".
    try:
...
#TK10 Проверка Формы "Регистрация" на наличие основных элементов.
    try:
        page_reg = RegistrPage(selenium)
...
#TK11 Проверка Формы "Регистрация" на соответствие названий элементов блока требованию.
    try:
        page_reg = RegistrPage(selenium)
...
#TK12 Проверка Регистрации пользователя с валидными данными: "Имя" и "Фамилия" на кириллице.
    page_reg = RegistrPage(selenium)
...
#TK13 Проверка на уникальность введенного e-mail в форме "Регистрация".
    page_reg = RegistrPage(selenium)
    page_reg.first_name.send_keys(Settings.first_name)
    page_reg.first_name.clear()
    page_reg.last_name.send_keys(Settings.last_name)
...
#TK14 Тест формы "Авторизация" после нажатия кнопки "Войти" при регистрации пользователя e-mail,
...
#TK15 Тест поля ввода "Имя" формы "Регистрация" допустимыми валидными значениями:
...
@pytest.mark.parametrize("invalid_first_name",
                         ...
                         [
                             ...
                             'empty', 'numbers', 'latin_symbols',  'special_symbols'
                         ])

def test_first_name_invalid_data(selenium, invalid_first_name):
#TK16 Тест поля ввода "Имя" формы "Регистрация" невалидными значениями: пустое значение;
...
#TK17 Тест поля ввода "Пароль" формы "Регистрация" валидными значениями.:
...
#TK18 Тест поля ввода "Пароль" и "Подтвердить пароль"
# формы «Регистрация» валидными значениями(пароли совпадают).
    page_reg = RegistrPage(selenium)
...
#TK19 Тест поля ввода "Пароль" и "Подтвердить пароль" формы
# «Регистрация» невалидными значениями(пароли не совпадают).
    page_reg = RegistrPage(selenium)
    page_reg.password_registration.send_keys(Settings.passw1)
    page_reg.password_registration.clear()
